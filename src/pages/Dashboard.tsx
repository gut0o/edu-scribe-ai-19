...
import { currentUser, getSubjectsByRole } from '@/lib/mock-data';
import { Subject } from '@/types';

const Dashboard = () => {
  const [user] = useState(currentUser);
  const subjects = getSubjectsByRole(user.role);
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
          ...
          <StatsCard
            ...
            value={subjects.reduce((acc, s) => acc + (s._count?.sources || 0), 0)}
            icon={FileText}
            description="PDFs, PPTs e imagens"
          />
          <StatsCard
            title="Perguntas Feitas"
            value={subjects.reduce((acc, s) => acc + (s._count?.chatMessages || 0), 0)}
            icon={MessageSquare}
          ...
          <div className="flex items-center justify-between">
            <TabsList>
              <TabsTrigger value="subjects">
                {user.role === 'PROFESSOR' ? 'Minhas Matérias' : 'Matérias Disponíveis'}
              </TabsTrigger>
              <TabsTrigger value="recent">Atividade Recente</TabsTrigger>
            </TabsList>
            
            {user.role === 'PROFESSOR' && (
              <HeroButton variant="hero" className="gap-2">
          ...
          <TabsContent value="recent" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Últimas interações e atualizações
                </CardDescription>
              </CardHeader>
              <CardContent>
